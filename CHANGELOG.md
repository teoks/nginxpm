...
- **Backend Security & Route Hardening (`backend/`)**:
  - **Rate Limiting**: Introduced `express-rate-limit` middleware (`authLimiter`, `certDownloadLimiter`) across `/tokens`, `/tokens/2fa`, `/users/:user_id/login`, and `/api/nginx/certificates/:certificate_id/download`.
...
- **Backend Dependencies (`backend/package.json`, `backend/yarn.lock`, `backend/package-lock.json`)**:
  ...
  - Upgraded `knex` to `3.3.0`, `body-parser` to `^2.3.0`, `mysql2` to `^3.23.4`, `otplib` to `^13.5.0`, `pg` to `^8.23.0`, `proxy-agent` to `^8.0.2`, `@apidevtools/json-schema-ref-parser` to `^16.0.0`.
...
- **Test Subsystem Dependencies & Resolutions (`test/package.json`, `test/yarn.lock`)**:
  - Added resolutions for `serialize-javascript` (`^7.0.5`, resolved to `7.1.0`), `underscore` (`^1.13.8`), and `diff` (`^8.0.3`, resolved to `8.0.4`), eliminating all transitive DoS/RCE advisories from Mocha and Cypress.
...
- **Test Dependencies (`test/yarn.lock`)**:
  - Upgraded `fast-uri` (`3.1.2` → `3.1.6`) resolving high-severity URI normalization and canonicalization advisories GHSA-5jgf-p345-68v8, GHSA-fph4-wmhf-6fwf, GHSA-f65p-4m7j-42xc, and GHSA-jqff-g426-hqxp (PR #32).
...
- **Access Control & Authorization (`backend/lib/access.js`)**:
  ...
  - Resolves an issue where authorization check failures threw `undefined`, causing requests to erroneously fall through to the catch-all router with a `404 Not Found` response instead of returning `403 Forbidden` (ported from upstream `ce4e35a3`).
...
- **Frontend Dependencies (`frontend/package.json`, `frontend/yarn.lock`)**:
  - Upgraded `@tabler/icons-react` to `^3.46.0`, `@tanstack/react-query` to `^5.101.4`, `react` & `react-dom` to `^19.2.8`, `react-router-dom` to `^7.18.2`, `typescript` to `7.0.2`, `vite` to `^8.2.2`, `vitest` to `^4.1.11`.
...
- **Docker Build Fix (`docker/Dockerfile`)**:
  - Installed `build-essential` and `python3-dev` build tooling temporarily around `yarn install` so `node-gyp` can compile native C/C++ addons (e.g. `better-sqlite3` 13.0.3) without adding unnecessary image layers.
...
- **Certbot Plugin Installer (`backend/lib/certbot.js`)**:
  ...
  - Added dynamic detection for Python virtual environments: uses `pipx inject certbot` when Certbot is installed via `pipx` (`/opt/pipx`), falling back to legacy `/opt/certbot/bin/activate` if present.
...
- **Base OS (`debian:trixie-slim`)**: Replaced the custom, opaque `nginxproxymanager/nginx-full` base image with official `debian:trixie-slim` (Debian 13) for full transparency, auditability, and GPG-signed package verification.
...
- **CoreDNS Replacement**: Replaced `jc21/dnsrouter` test sidecar in `docker-compose.ci.yml` and `docker-compose.dev.yml` with official CNCF `coredns/coredns:latest` and created `docker/dev/Corefile`.
