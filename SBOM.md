...
| **Cryptography / TLS** | OpenSSL `3.5.6-1~deb13u2` / `libssl3t64` | Debian Trixie security repository | TLS 1.2/1.3 cryptographic engine |
| **Process Supervisor** | s6-overlay `v3.2.3.2` | `just-containers/s6-overlay` | Multi-process container init supervisor |
| **Certificate Client** | Certbot via `pipx` (DNS plugins) | Python Packaging Index (`pipx`) | Automated ACME Let's Encrypt certificates |
...
| `objection` | `3.1.5` | Lightweight ORM for Knex.js |
...
| `express-rate-limit` | `^8.6.2` | Endpoint abuse & brute-force rate limiter |
...
| `@apidevtools/json-schema-ref-parser` | `^16.0.0` | JSON Schema $ref resolution |
| `archiver` | `^8.0.0` | File and archive streaming utility |
| `express-fileupload` | `^1.5.2` | Multi-part form/file upload middleware |
| `node-rsa` | `^2.0.0` | RSA cryptography and key generation |
| `@biomejs/biome` | `^2.5.9` | Linting and code formatting (Dev) |
...
| `react-intl` | `^10.1.22` | Internationalization & localization |
| `react-markdown` | `^10.1.0` | Markdown rendering component |
| `react-select` | `^5.10.2` | Select dropdown UI control |
| `react-toastify` | `^11.1.0` | Toast notifications system |
| `@uiw/react-textarea-code-editor` | `^3.1.1` | Code editor textarea component |
| `vitest` | `^4.1.11` | Unit & integration test runner (Dev) |
| `@biomejs/biome` | `^2.5.10` | Linting and code formatting (Dev) |
...
| :--- | :--- | :--- |
| `cypress` | `^15.15.0` | End-to-End browser API test automation |
| `@quobix/vacuum` | `^0.30.0` | OpenAPI / Swagger linting & validation |
| `mocha` | `^11.7.5` | Test reporting framework |
| `@jc21/cypress-swagger-validation` | `^0.3.2` | Response schema validation plugin |
| `axios` | `^1.18.0` | Promise-based HTTP client for API testing |
| `eslint` | `^10.3.0` | Linter for test suites |

---

## 5. Documentation Subsystem (`docs/`)

| Tool / Dependency | Version Range | Purpose |
| :--- | :--- | :--- |
| `vitepress` | `^1.6.4` | Static documentation site generator |
| `vite` | `^6.4.3` | Bundler runtime (resolution) |

---

## 6. SBOM Generation Command Reference
