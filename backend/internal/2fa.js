...
const internal2fa = {
	...
	verifyForLogin: async (userId, token) => {
		...
		const auth = await internal2fa.getUserPasswordAuth(userId);
		const secret = auth?.meta?.totp_secret;
		...
		// Attempt TOTP code verification
		try {
			const result = await verify({
				token: cleanToken,
				secret,
			...
			// Token was not a valid TOTP code, proceed to check backup codes
		}
		...
		if (Array.isArray(backupCodes) && backupCodes.length > 0) {
			for (let i = 0; i < backupCodes.length; i++) {
				const match = await bcrypt.compare(cleanToken.toUpperCase(), backupCodes[i]);
				if (match) {
