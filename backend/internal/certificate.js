...
const internalCertificate = {
	...
	validate: (data) => {
		...
		const allowedFiles = internalCertificate.allowedSslFiles;
		const files = {};
		for (const name of allowedFiles) {
			if (data.files?.[name]?.data) {
				files[name] = data.files[name].data.toString();
			}
		}
	...
	requestLetsEncryptSsl: async (certificate, email) => {
		...
		logger.info(`Command: ${certbotCommand} ${args.join(" ")}`);
	...
	requestLetsEncryptSslWithDnsChallenge: async (certificate, email) => {
		...
		fs.mkdirSync(path.dirname(credentialsLocation), { recursive: true });
		fs.writeFileSync(credentialsLocation, certificate.meta.dns_provider_credentials, {
			encoding: "utf8",
			mode: 0o600,
		});

		const certbotFlags = await getPluginCertbotFlags(certificate.meta.dns_provider);
		...
		const args = [
			...
			"--agree-tos",
			"--authenticator",
			certbotFlags.authenticator,
			`--${certbotFlags.authenticator}-credentials`,
			credentialsLocation,
			"--non-interactive",
			"--preferred-challenges",
			"dns",
		...
		// Add propagation seconds parameter if specified
		if (certificate.meta?.propagation_seconds) {
			args.push(`--${certbotFlags.authenticator}-propagation-seconds`, certificate.meta.propagation_seconds);
		}
	...
	revokeLetsEncryptSsl: async (certificate, throwErrors) => {
		...
		try {
			const result = await utils.execFile(certbotCommand, args, adds.opts);
			await fs.promises.rm(`/etc/letsencrypt/credentials/credentials-${Number(certificate.id)}`, { force: true });
			logger.info(result);
			return result;
		} catch (err) {
			logger.error(err.message);
		...
		if (Array.isArray(payload?.domains)) {
			for (const domain of payload.domains) {
				if (typeof domain === "string" && domain.length > 0 && domain.length < 255) {
					const testRes = await internalCertificate.performTestForDomain(domain);
					resultsMap.set(domain, testRes);
				}
			}
		}
