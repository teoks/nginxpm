...
import { certDownloadLimiter } from "../../lib/express/rate-limit.js";
...
router.param("certificate_id", (req, _res, next, id) => {
	const parsedId = Number.parseInt(id, 10);
	if (!parsedId || Number.isNaN(parsedId)) {
		return next(new errs.ValidationError("Invalid certificate ID"));
	}
	req.certificateId = parsedId;
	next();
});
...
router
	...
	.get(async (req, res, next) => {
		try {
			const expandFields = typeof req.query.expand === "string"
				? req.query.expand.split(",").filter((f) => /^[a-zA-Z_]+$/.test(f))
				: null;
			const data = await apiValidator(
				{
					type: "object",
...
router
	 ...
	 */
	.get(certDownloadLimiter, async (req, res, next) => {
		try {
